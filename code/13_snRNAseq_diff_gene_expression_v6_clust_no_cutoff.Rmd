---
title: "13_snRNAseq_diff_gene_expression_v6_clust_no_cutoff"
...
start_data <- "QCd_louvain_multi_clustered_harmony_ws_32pcs_ePRS_44s_uns_20250130.rdata"
...
overwrite <- TRUE
    ...
    # leave as NULL
    write_name <- NULL #"louvain_mast_cc"
...
# Choose which DEG tests to run_-----------------------------------------------
  ...
  clust_to_specific_clust <- FALSE
    ...
    base_clust <- NULL #c(0)
    comp_clust <- NULL #c(1:11) #c(3, 4, 5, 6, 7, 9, 10)
...
# Set parameters for all DEG test(s) below-------------------------------------
  ...
  meta_vars <- NULL #"orig.ident"  #"treatment"
...
if (!exists("cluster_idents")){
  ...
  cluster_idents <- sort(unique(Idents(ss_data_norm)))
  cluster_idents <- as.integer(levels(cluster_idents))
} 
...
if(GSEA) {
  ...
  run_gsea <- function(pathway, results) {
    user_GSEA(results, TERM2GENE = pathway, seed = T) %>% arrange(p.adjust)
  }
...
plot_vol <- function(markers, type) {
  ...
  top_genes <- top_n(markers, n = 10, wt = avg_log2FC)
  top_genes <- rbind(top_genes, top_n(markers, n = -10, wt = avg_log2FC))
...
if (all_cells_by_group) {
  ...
  for (group in 1:length(meta_vars)) {
    ...
    group_markers <- filter(group_markers,
                            abs(group_markers$avg_log2FC) > ln_threshold)
    print(head(group_markers, n=20))
    top10_genes <- row.names(top_n(group_markers, n = 10, wt = avg_log2FC))
    bottom10_genes <- row.names(top_n(group_markers, n = -10, wt = avg_log2FC))
...
if (clust_to_specific_clust) {
  DEG_path <- change_path(paste0(outpath, "/DEG/cluster_to_cluster/no_cutoff/"))
  vln_path <- change_path(paste0(DEG_path, "vln_plots/"))
  vol_path <- change_path(paste0(DEG_path, "vol_plots/"))
  heat_path <- change_path(paste0(DEG_path, "heatmap_plots/"))
    ...
    for (cluster in length(comp_clust):1) {
      if (comp_clust[cluster] != base_clust[index] &
        ...
        sig_markers <- filter(sig_markers,
                              abs(sig_markers$avg_log2FC) > ln_threshold)
        if (dim(sig_markers)[1] > 0) {
          print(head(sig_markers, n=20))
          top10_genes <- row.names(top_n(sig_markers, n = 10, wt = avg_log2FC))
          bottom10_genes <- row.names(top_n(sig_markers, n = -10, wt = avg_log2FC))
...
if (clust_to_all_clust) {
  ...
  data_markers <- filter(data_markers,
                         abs(data_markers$avg_log2FC) > ln_threshold)
  ...
  if (GSEA) {
    ...
    # Run GSEA using avg_logFC
   for (clusternum in 1:length(cluster_idents)){
        clust_markers <- filter(all_markers, cluster ==
                                   (cluster_idents[clusternum]))
    # Keep only gene symbol and stat value for pathway analysis
    clust_markers <- setDT(clust_markers, keep.rownames = "genes")
    ...
    # Run the gsea algorithm for each pathway
    for (pathway in pathway_list) {
       gsea_results <- run_gsea(get(pathway), clust_markers)
       if (dim(gsea_results@result)[1] > 0) {
         save_gsea(cluster_idents[clusternum],
                   gsea_results, pathway, chose_resolution) 
         }
      }
    }
...
if (clust_to_all_clust) {
  ...
  for (genes in c(3,5,10)) {
    list <- data_markers %>% group_by(cluster) %>%
            top_n(n = genes, wt = avg_log2FC) %>% pull(var = gene) %>% unique()
...
if (clust_to_all_clust) {
  ...
  plot_heatmap <- function(features) {
    ss_data_avg <- AverageExpression(ss_data_norm, features = get(features),
                                   assays = DEG_assay, return.seurat = F)
