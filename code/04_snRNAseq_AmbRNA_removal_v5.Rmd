...
author: "Kevin Green & Katie Prater & Kevin Z Lin"
date: "01/24/2024"
output:
  html_document:
    df_print: paged
...
start_data <- "UMAP_ws_ePRS_44s_uns_20250130.rdata"

# Set resolution to the clustering resolution you want from your script 03 output
# i.e. if you feel the 0.4 resolution umaps fit all samples best, put "0.4" here
# This should be a character variable, so it needs quotes around the value.
clust_res <- "0.4"
...
genes_to_explore <- c("MT-CO2", "S100B") #"IGKC", "S100B", "HBA1", "HBB",
...
# Generate the resolution name from the clust_res variable supplied by user
resolutions <- paste0(assay, "_snn_res.", as.character(clust_res))
index <- 1

# Print statement to let the user know what clustering resolution is used
...
for (file in 1:length(samples_path)) {
  ...
  count_mat <- ss_data_split[[file]]@assays$RNA@counts ## KZL: WARNING: This will NOT work for Seurat v5. For Seurat v5, please use instead:
  # count_mat <- SeuratObject::LayerData(ss_data_split[[file]], assay = "RNA", layer = "counts")
  
  ###############
  # [[Kevin Z Lin's edit: Make sure sparse_data and count_mat have the same genes
  #  in the same order. To do this:
  #  1) For any genes in count_mat NOT in sparse_data, add to sparse_data row of all 0's 
  #  2) For any genes in sparse_data NOT in count_mat, remove the gene from sparse_data]]
  
  gene_vec <- rownames(count_mat)
  stopifnot(!any(duplicated(gene_vec)))
  stopifnot(!any(duplicated(rownames(sparse_data))))
  
  # handle missing genes in sparse_data
  missing_genes <- gene_vec[which(!gene_vec %in% rownames(sparse_data))]
  if(length(missing_genes) > 0){
    gene_vec_alt <- rownames(sparse_data)
    sparse_data <- rbind(sparse_data, 
                         Matrix::Matrix(data = 0,
                                        nrow = length(missing_genes),
                                        ncol = ncol(sparse_data),
                                        sparse = T))
    rownames(sparse_data) <- c(gene_vec_alt, missing_genes)
  }
  
  # handle extra genes in sparse_data, and also reorganize genes in sparse_data
  sparse_data <- sparse_data[gene_vec,]
...
### This code will explore the genes that were most affected by
...
for (file in 1:length(soupx_out)){
  ...
  seurat_obj[["Sample_ID"]] <- names[file]
