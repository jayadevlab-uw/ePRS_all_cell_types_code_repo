---
title: "JayadevLab_snRNAseq_Pipeline_QC_portion_v2.1"
...
### Take raw count files from Cell Ranger and read them into Seurat objects.
...
if (numsubs == length(unique(ss_data@meta.data$Sample_ID))){
...
# Save the data alone as a separate file
...
# Split the Seurat object so that each sample can be processed individually
ss_data_split <- SplitObject(ss_data, split.by = "orig.ident")

# Make the same plot for each sample individually
for (sample in 1:length(ss_data_split)) {
    plot1 <- FeatureScatter(object = ss_data_split[[sample]], feature1 = "nCount_RNA",
                        feature2 = "percent.mito") + ggtitle(names[sample]) +
                        theme(legend.position = "none")
    plot1
    ggsave(filename = paste0(plotspath, names[sample], "_UMI_vs_mito_scatterplot",
                             suffix, ".png"), height = 9, width = 9, plot = plot1)
}
...
plot3 <- ggplot(ss_data@meta.data, 
                aes(color=ss_data@meta.data$Sample_ID, x=nCount_RNA,
                    fill=ss_data@meta.data$Sample_ID)) + 
         geom_density(alpha = 0.2) + 
         scale_x_log10() + 
         theme_classic() +
         xlab("Number of copies/gene (nCount)") +
         ylab("Frequency") +
...
# Make plot 3 for each sample individually
for (sample in 1:length(ss_data_split)) {
    plot6 <- ggplot(ss_data_split[[sample]]@meta.data, 
                aes(x=nCount_RNA)) + 
         geom_density(alpha = 0.2) + 
         scale_x_log10() + 
         theme_classic() +
         ...
         geom_text(mapping = aes(x = 200,
          ...
          geom_text(mapping = aes(x = 350,
                          ...
                          hjust = -1.5,
                          vjust = .6) + 
          ggtitle(names[sample]) + theme(legend.position = "none") + 
          coord_cartesian(xlim = c(100,100000), ylim = c(0,8))
    plot6
    ggsave(filename = paste0(plotspath, names[sample], "_QC_num_copies_gene",
                             suffix, ".png"), height = 9, width = 9, plot = plot6)
}

plot4 <-  ggplot(ss_data@meta.data,
                 aes(color=ss_data@meta.data$Sample_ID, x=percent.mito,
                     ill=ss_data@meta.data$Sample_ID)) + 
          geom_density(alpha = 0.2) + 
          scale_x_log10() + 
          theme_classic() +
...
plot5 <-  ggplot(ss_data@meta.data, 
                 aes(color=ss_data@meta.data$Sample_ID, x=nFeature_RNA, 
                     fill=ss_data@meta.data$Sample_ID)) + 
      geom_density(alpha = 0.2) + 
      theme_classic() +
      scale_x_log10() +
      xlab("Number of genes/cell (nFeature)") +
      ylab("Frequency") +
         geom_vline(xintercept = 200) +
         geom_vline(xintercept = 300) +
         geom_text(mapping = aes(x = 200,
...
# Make plot 5 individually for each sample
for (sample in 1:length(ss_data_split)) {
    plot6 <- ggplot(ss_data_split[[sample]]@meta.data, 
                aes(x=nFeature_RNA)) + 
         geom_density(alpha = 0.2) + 
         scale_x_log10() + 
         theme_classic() +
         xlab("Number of copies/gene (nFeature)") +
         ylab("Frequency") +
         geom_vline(xintercept = 200) +
         geom_vline(xintercept = 300) +
         geom_text(mapping = aes(x = 200,
          ...
          coord_cartesian(xlim = c(100,100000), ylim = c(0,8))
    plot6
    ggsave(filename = paste0(plotspath, names[sample], "_QC_num_genes_cell",
                             suffix, ".png"), height = 9, width = 9, plot = plot6)
}

# Create a combined QC plot
plot7 <- CombinePlots(plots = list(plot3,plot4,plot5))
ggsave(filename = paste0(plotspath, "QC_feature_histograms_by_sample",
                        suffix, ".png"),
       height = 9, width = 18, plot = plot7)
