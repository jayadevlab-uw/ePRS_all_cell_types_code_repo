---
title: "11.5_snRNAseq_cluster_stats_optional_subset_clusters_v6_harmony"
...
start_data <- "QCd_louvain_multi_clustered_harmony_ws_32pcs_harm_44s_uns_20250130_rm813_13to16_2627.rdata" 
...
change_output_path <- "../output/output_mg_subset_rm_s06s11/allADNC_cohort/"
...
meta_vars <- c("study_designation", "APOE4_status", "sex", "frozen_tissue_type")
...
change_suffix <- "_harm_36s_uns_20250130_rm813_13to16_2627_allADNC"

# If sample_sub = TRUE: are you using clusters or metadata as your subset
# variable? Input "clusters" or "metadata" here.
subset_type <- "metadata"

# Do you want to save the inverse of the below list in your Seurat object?
# If so, set to TRUE. If you want to save the list you made, put FALSE.
invert = FALSE
...
#If you are subsetting by METADATA:
  ...
  sub_meta_var <- "Sample_ID"

  # If sample_sub = TRUE: Give specific group name(s) to subset on 
  # If False, give NULL.
  meta_subset_names <- c("s01", "s02", "s04", "s05", "s06", "s07", "s08", "s09",
                         "s10", "s11", "s12", "s14", "s15", "s17", "s18", "s19", 
                         "s20", "s21", "s22", "s23", "s24", "s25", "s26", "s27", 
                         "s28", "s29", "s30", "s31", "s32", "s33", "s35", "s36", 
                         "s37", "s43", "s54", "s71")
...
if (sample_sub) {
  ...
  print(paste("The number of prinicple components where the variation gets small is:",
              PCs))
...
if (sample_sub) {
  ...
  rm(overall, prop_overall, subset_table, sample_table, nPCs,
     ...
     clusters_prefix, cnam, snns, invert, red, variances, PCs)

  #Save workspace image for the subset dataset:
  print(paste0("Saving subset dataset in: ", rfiles_path,
               "QCd_norm_harmony_ws", suffix, ".rdata"))
  save.image(file = paste0(rfiles_path, "QCd_norm_harmony_ws",
