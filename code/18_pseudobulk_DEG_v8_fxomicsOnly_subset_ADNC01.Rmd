...
start_data <- "QCd_louvain_multi_clustered_harmony_ws_32pcs_harm_44s_uns_20250130_rm813_13to16_2627_ADNC01_ws_rm6808.rdata"
  #"QCd_louvain_multi_clustered_harmony_ws_34pcsharm_23s_uns_ADNChigh_rm_8_14.rdata"
#QCd_louvain_multi_clustered_harmony_ws_34pcsharm_23s_uns_ADNChigh_rm_8_14.rdata"
# "QCd_louvain_multi_clustered_harmony_ws_34pcs_harm_21s_uns_ADNChigh_noAPOE44.rdata"
###*** This version for the ADNC01 has extra code to add the eprs_group variable to the metadata of the object.
...
change_csv <- "ePRS_13samples_ADNC01_unsorted_metadata_20250714.csv"
...
meta_vars <- c("ePRS_group", "sex", "APOE4_status")
...
interactions <- NULL #c("study_designation:sex")

# Clusters to remove:
remove <- NULL

# If you are subsetting by a metadata variable/group -  
  # provide the variable name as seen in your Seurat object metadata.
  subset_type <- NULL #"Sample_ID"

  # Give specific group name(s) to subset on 
  # If False, give NULL.
  subset_names <- NULL #c("s16")
  

# Input any metadata variables for discrete groups that are set as integers
# (e.g. mutation status = 0 or 1). Otherwise, NULL
meta_factors <- c("APOE4_status")
...
combined <- #c("0", "3") # Oligos
  #c("6", "14") # PV/SST + Chandelier
  #c("10", "13", "15", "18") # Deep neurons layer 5/6
  #c("11", "21") #Layer 4/5 ET
  c("1", "9", "12") #Layer 2/3
  #c("0", "3", "16", "19") # Oligos
  #c("1", "9", "10", "11", "12", "13", "15", "18", "21", "24") #ExcNeurons
  #c("6", "7", "8", "14") #Inh.Neurons

# Rdata ouput name - add an identifier to your rdata file for ease of reuse
rdata_out_ident <- "ePRS_ADNC01_13samples_20260106_layer23"
...
if (!is.null(change_resolution)) {
  ...
  res_name <- paste0("SCT_snn_res.", chose_resolution)
  
  Idents(object = ss_data_norm_subset) <- res_name
}

# Check if meta.data variables match gene names
meta_data <- colnames(ss_data_norm_subset@meta.data)
all_genes <- row.names(ss_data_norm_subset@assays$RNA@counts)
while (any(meta_data %in% all_genes)) {
  ...
  warning(paste0("Changing the name of ", change, " metadata column to ",
                 change, "_status"))
  ss_data_norm_subset@meta.data[[paste0(change, "_status")]] <- ss_data_norm_subset@meta.data[[change]]
  ss_data_norm_subset@meta.data[[change]] <- NULL
  meta_data <- colnames(ss_data_norm_subset@meta.data)
}

# Change the active assay to the assay to be evaluated for DEG
DefaultAssay(ss_data_norm_subset) <- "RNA"
```
...
bulk_path <- change_path(paste0(DEG_path, "pseudobulk_", rdata_out_ident, "/"))
full_path <- change_path(paste0(bulk_path, "no_cutoff/"))
...
#     print("Subsetting:")
#     print(subset_names)
#     Idents(ss_data_norm_subset) <- subset_type
#     ss_data_norm_subset <- subset(ss_data_norm_subset, idents = subset_names, 
#                                   invert = TRUE)
#     print("The new dimensions of the subset dataset are:") 
#     print(dim(ss_data_norm_subset))
#   }
#}
# 
# Remove any unwanted clusters and subset the clusters to combine
if (!is.null(remove)) {
  ss_data_norm <- subset(ss_data_norm_subset, idents = remove, invert = TRUE)
#}else if (!is.null(combined)){
#  Idents(ss_data_norm_subset) <- res_name
#  ss_data_norm <- subset(ss_data_norm_subset, idents = combined, invert = FALSE)
} else {
  print("No clusters were removed from the dataset.")
}

# Extract raw counts and metadata to create SingleCellExperiment object
counts <- ss_data_norm_subset@assays$RNA@counts
metadata <- ss_data_norm_subset@meta.data

# Subset the counts matrix by the clusters to be combined.
# This is instead of the subset above - not sure why the subset wasn't working.
metadata <- metadata %>% dplyr::filter(get(res_name) %in% combined)
counts <- counts[,row.names(metadata)]

# # Add the metadata for ePRS category
# # Sample_ID <- c("s02", "s07", "s08", "s14", "s23", "s24", "s30", "s31", "s33", "s35", "s36", "s37", "s54")
# # ePRS_group <- c("Low", "Low", "Low", "High", "Low", "Low", "High", "High", "High", "High", "Low", "Low", "High")
# # Combine into a data frame
# eprs_cat <- tibble(Sample_ID, ePRS_group)
# metadata <- left_join(metadata, eprs_cat, "Sample_ID")


# Check which variables from your regression are already in your metadata
to_remove <- meta_vars %in% colnames(metadata)
# ## *** Added to avoid removing the ePRS_group variable since it isn't in the metadata
# to_remove[1] <- FALSE
...
min_nuclei <- min(table(Idents(ss_data_norm_subset))[table(Idents(ss_data_norm_subset)) > 0])
...
if (!is.null(combined)) {
  ...
  metadata$cluster_id <- factor(ss_data_norm_subset@active.ident)
}
rm(ss_data_norm_subset)
...
gg_df <- data.frame(cluster_id = de_cluster_ids,
                    sample_id = de_samples) #str_c("s", de_samples))
names(gg_df)[2] <- ID

# Check if the metadata df has a Sample_ID that is not characters. - Not flexible**
if (!identical(metadata_df[ID],gg_df[ID])){
  print("Altering data type to match.")
  metadata_df[,ID] <- csv_metadata[ID]
}

# Put the dataframes together.
gg_df <- left_join(gg_df, metadata_df[, c(ID, meta_vars)])
metadata <- gg_df %>% dplyr::select(cluster_id, all_of(ID), all_of(meta_vars))
...
KEGG_pathways <- read.gmt("../data/c2.cp.kegg_medicus.v2024.1.Hs.symbols.gmt")
React_pathways <- read.gmt("../data/c2.cp.reactome.v2024.1.Hs.symbols.gmt")
WP_pathways <- read.gmt("../data/c2.cp.wikipathways.v2024.1.Hs.symbols.gmt")
GO_BP_pathways <- read.gmt("../data/c5.go.bp.v2024.1.Hs.symbols.gmt")
GO_MF_pathways <- read.gmt("../data/c5.go.mf.v2024.1.Hs.symbols.gmt")
GO_CC_pathways <- read.gmt("../data/c5.go.cc.v2024.1.Hs.symbols.gmt")

pathway_list <- c("KEGG_pathways", "React_pathways", "WP_pathways",
                  "GO_BP_pathways", "GO_MF_pathways", "GO_CC_pathways")
...
save_gsea <- function(cluster, effect, gsea, path_name, res) {
  ...
  #                          "_chart_", effect, "_res", res, suffix, ".pdf"),
  ...
  #                            "_eplot_", effect, "_res", res, suffix, ".pdf"),
...
heatmap_colors <- colorRampPalette(c("purple4", "white", "gold3"))
...
for (cluster in clusters) {
  ...
  cluster_counts <- as.data.frame(sub_counts[, which(colnames(sub_counts) %in% rownames(cluster_metadata))]) #str_c("s", colnames(sub_counts)) %in%
  ...
  for (meta_var in meta_vars) {
    ...
    # Make cluster_counts match cluster_metadata
    cluster_counts <- cluster_counts[,rownames(cluster_metadata)]
  ...
  # Reorder the data by metadata group for visualization
  cluster_metadata <-
    cluster_metadata[order(as.vector(cluster_metadata[[meta_var]]),
                                     cluster_metadata[[ID]]),]
  ...
  for (effect in names) {
    ...
    sig_markers <- filter(results,
                          padj < 0.1 & abs(log2FoldChange) > log2_threshold)
    print(sig_markers)
    ...
    if (dim(sig_markers)[1] > 0) {
      ...
      #                          "_res.", chose_resolution, suffix, ".pdf"),
      ...
      write.csv(results,
                paste0(full_path, cluster, "_pseudo_all_genes_", effect,
                       "_res", chose_resolution, suffix, ".csv"))
      write.csv(filter(results,
                       padj < 0.1 & abs(log2FoldChange) > log2_threshold),
                paste0(bulk_path, cluster, "_pseudo_DEGs_", effect,
                       "_res", chose_resolution, suffix, ".csv"))
      sig_markers <- setDT(sig_markers, keep.rownames = TRUE)
    
      # Create fitted value matrix
      fitted <- assays(DEG_data)[["mu"]]
      assign(paste0("fitted_", cluster, "_", effect), fitted)
      
      # Get the top and bottom DEGs by log2 fold change
      # n <- min(100, nrow(sig_markers))
      top10_genes <- top_n(subset(sig_markers, log2FoldChange > 0),
                           n = 20, wt = -padj)
      bottom10_genes <- top_n(subset(sig_markers, log2FoldChange < 0),
                              n = 20, wt = -padj)
      top_genes <- rbind(arrange(top10_genes, log2FoldChange),
                         arrange(bottom10_genes, log2FoldChange))
      fitted <- subset(fitted, row.names(fitted) %in% top_genes$row)
      fitted <- fitted[,as.character(cluster_metadata[[ID]])]
      if (nrow(sig_markers) > 1) {
        fitted <- fitted[match(top_genes$row, rownames(fitted)),]
        ...
        ggsave(filename = paste0(plots_path, cluster, "_pseudo_DEG_heatmap_", effect,
                                 "_res", chose_resolution, "_", suffix, ".pdf"),
               height = 7, width = 11, plot = heatmap)
      }
    }
    ...
    if (identical(method, "lfc") & dim(filter(results, padj < 0.1))[1] > 0) {
      ...
      for (pathway in pathway_list) {
        ...
        save_gsea(paste0(cluster, "_all_paths"), effect, gsea_results, pathway,
                  chose_resolution)
        if (dim(gsea_results@result)[1] > 0) {
          ...
          assign(paste0("gsea_results_", cluster, "_", pathway, "_", effect),
               gsea_results)
          save_gsea(paste0(cluster, "_sig_paths"), effect, gsea_results, pathway,
                    chose_resolution)
          if (dim(gsea_results@result)[1] > 20) {
            gsea_results@result <- gsea_results@result %>%
            filter(!between(dense_rank(NES), 11, n() - 10))
...
if (is.null(combined)) {
  save.image(file = paste0(rfiles_path, "pseudobulk_ws", rdata_out_ident, suffix,
                           ".rdata"), compress = TRUE)
} else {
  save.image(file = paste0(rfiles_path, paste(combined, collapse = "-"),
                           "_pseudobulk_ws", rdata_out_ident, suffix, ".rdata")
             , compress = TRUE)
}
```
