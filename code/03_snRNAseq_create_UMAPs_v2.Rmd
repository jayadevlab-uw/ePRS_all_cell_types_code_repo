...
author: "Kevin Green & Katie Prater"
date: "10/17/2022"
output:
  html_document:
    df_print: paged
...
start_data <- "preQC_normalized_ws_ePRS_44s_uns_20250130.rdata"

# Adding assay as it was previously undefined in this set of scripts *needs fixing*
assay <- "SCT"

# Choose default resolutions to test (leave this on 1 unless you see you need
# the lower ones) 1 = 0.4, 0.6, 0.8, and 1.0 resolution & 2 = 0.1, 0.2, 0.3, 0.4
# for clustering of individual samples
umap_clust_res = 2
...
for (file in 1:length(ss_data_split)) {
  ...
  print(paste("The number of prinicple components where change diminishes is:",
              PCs[[file]]))
}
...
find_neighbors <- function(data) {
  foreach (file = 1:length(data)) %do% {
    print(paste("Finding nearest neighbors for sample:", names[file]))
...
# Set up the clustering resolutions based on user choice in setup variables
if(umap_clust_res == 1) {
  clust_res <- c(0.4, 0.6, 0.8, 1.0)
} else {
  clust_res <- c(0.1, 0.2, 0.3, 0.4)
}
...
resolutions <- c(paste0(assay, "_snn_res.", as.character(clust_res[1])), 
                 paste0(assay, "_snn_res.", as.character(clust_res[2])), 
                 paste0(assay, "_snn_res.", as.character(clust_res[3])), 
                 paste0(assay, "_snn_res.", as.character(clust_res[4])))

umap_pngs <- c(paste0("UMAP_", as.character(clust_res[1])),
               paste0("UMAP_", as.character(clust_res[2])),
               paste0("UMAP_", as.character(clust_res[3])),
               paste0("UMAP_", as.character(clust_res[4])))
...
```{r Save Data}
# Remove files that are no longer needed
rm(file, num, obj, pcs, PCs, umap_pngs, find_clusters, find_neighbors, identify_clusters, umap_clust_res, clust_res, resolutions)
