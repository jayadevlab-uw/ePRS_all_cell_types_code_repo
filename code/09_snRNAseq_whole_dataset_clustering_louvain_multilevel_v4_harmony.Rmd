---
title: "09_snRNAseq_whole_dataset_clustering_louvain_multilevel_v4"
...
## Clustering using the Louvain algorithm with multilevel refinement
...
start_data <- "QCd_norm_harmony_ws_harm_42uns.rdata"
...
change_output_path <- NULL #"../output/output_clust_inclusive_harmony/"

# Change the suffix, if necessary
change_suffix <- NULL #"_harm_23s_uns_highADNC_ePRS_rm_8_14"
...
red <- na.omit(tolower(str_extract(start_data, c("harmony", "MNN", "anchor"))))
...
# Set up resolutions to be tested during clustering
res <- c(0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7,
         0.75, 0.8)
...
ss_data_norm <- FindClusters(ss_data_norm,
                  resolution = res,
                  algorithm = 2, # 2 = Louvain algorithm with multilevel
                                 #     refinement
...
rm(plot1, plot2, den_plot, color_list, plot3, plot4, resolutions, umap_pngs,
   ...
   ninety_five, change_nPCs, params)
# Save workspace image if needed to revisit
save.image(file = paste0(rfiles_path, "QCd_louvain_multi_clustered_harmony_ws_", nPCs,
                         "pcs", suffix, ".rdata"), compress = TRUE)
```
